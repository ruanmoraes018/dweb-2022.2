...
class Passageiro(AbstractBaseUser, PermissionsMixin):
    user = models.OneToOneField(Custom, on_delete=models.CASCADE, primary_key=True)
    ...
    groups = models.ManyToManyField(Group, verbose_name=_('grupos'), blank=True, related_name='passageiros')
...
# Abaixo fica os modelos de Motorista
...
class Motorista(AbstractBaseUser, PermissionsMixin):
    user = models.OneToOneField(Custom, on_delete=models.CASCADE, related_name='motorista', primary_key=True)
    username = models.CharField(max_length=50, unique=True, verbose_name='Nome de Usuário')
    nome_completo = models.CharField(max_length=180, verbose_name='Nome Completo')
    nome_preferencia = models.CharField(max_length=50, verbose_name='Apelido')
    ...
    groups = models.ManyToManyField(Group, verbose_name=_('grupos'), blank=True, related_name='motoristas')
    user_permissions = models.ManyToManyField(
        Permission,
        verbose_name=_('permissões do usuário'),
        blank=True,
...
class RotaViagem(models.Model):
    ...
    def __str__(self):
        if self.ida_volta:
            return f"Viagem de ida e volta do {self.motorista.nome_preferencia} de {self.origem} para {self.destino} - Partida: {self.horario_ida.strftime('%d/%m/%Y %H:%M')} Retorno: {self.horario_volta.strftime('%d/%m/%Y %H:%M')}"
        else:
            return f"Viagem de ida do {self.motorista.nome_preferencia} de {self.origem} para {self.destino} - Partida: {self.horario_ida.strftime('%d/%m/%Y %H:%M')}"

    def save(self, *args, **kwargs):
        form_data = kwargs.pop('form_data', None)  # remova form_data dos argumentos kwargs
